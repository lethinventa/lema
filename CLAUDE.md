...
- **Produto** (`docs/product/`)
  ...
  - `interaction-patterns.md` — critério de quando usar página cheia vs. bottom sheet vs. modal centralizado para cada ação. Nível abaixo de `information-architecture.md` (estrutura vs. superfície de apresentação).
  - `visual-design-system.md` — paleta de marca, tipografia, raio, sombra, hierarquia de superfície e a regra de onde cor pode aparecer (só em tag/badge/estado selecionado, nunca em corpo de texto). Nível abaixo de `interaction-patterns.md` (como cada superfície é desenhada, não qual superfície usar).
