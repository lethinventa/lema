...
- **Acesso ao banco de dados (Drizzle) sempre através de um arquivo `<entidade>.repository.ts`** dentro de `features/<feature>/server/`, nunca diretamente de um `service.ts` — o service chama o repository, o repository chama `useDb()`/schema. **Um arquivo de repository é escopado por entidade, não por feature**: uma feature com mais de uma entidade própria (ex.: `groups` tem `Group`/`Membership` e também `Invitation`) tem um arquivo de repository por entidade (`group.repository.ts`, `invitation.repository.ts`), nomeado pela entidade, não um único repository cobrindo tudo. Uma função de repository só pode tocar as tabelas da entidade que ela representa — uma consulta a uma tabela de **outra** entidade (ex.: `users`) não fica ali só porque hoje é a única consumidora; ela vai no repository da feature dona daquela entidade (ex.: `features/users/server/users.repository.ts`), consumido por quem precisar através do `index.ts` da feature dona — mesma regra de fronteira que já vale pro resto do código de uma feature. Enforced via ESLint (`no-restricted-imports` bane `~/lib/db/*` fora de `*.repository.ts` dentro de `server/`; arquivos `*.test.ts` são isentos, já que testes legitimamente precisam de acesso direto ao banco para fixtures/assertions). Ver `apps/web/features/groups/server/group.repository.ts`, `apps/web/features/groups/server/invitation.repository.ts` e `apps/web/features/users/server/users.repository.ts` como referência.
