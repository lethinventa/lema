...
- **Goal**: objetivo pessoal ou compartilhado; relaciona-se com Tasks/Transactions/Events/Documents/Budgets/outros Goals (submetas).
  ...
  - **3 estados financeiros** por meta/submeta (além do custo estimado): `RESERVED` (reservado), `COMMITTED` (contratado, não totalmente pago), `PAID` (dinheiro efetivamente movimentado). `PAID` não é armazenado — é sempre a soma das Transactions que referenciam a Goal (`goalId`), ver GoalAllocation abaixo.
  - Quando a meta tem submetas, progresso = média automática do progresso das submetas (não editado manualmente).
- **GoalAllocation**: representa um valor em estado RESERVED ou COMMITTED ligado a uma Goal — nenhuma Transaction associada, porque o dinheiro ainda não se moveu. `PAID` NÃO é um estado de GoalAllocation: é sempre derivado da soma das Transactions com `goalId` = essa Goal (vincular a transação, em Finanças, já é o próprio ato de contabilizá-la como paga — não existe "marcar uma alocação como paga"). COMMITTED pode opcionalmente referenciar um Document. Custo estimado é um campo direto definido pelo usuário, NÃO calculado pela soma das alocações. Deletar uma submeta → suas alocações seguem o mesmo ciclo de lixeira de 30 dias; Transactions existentes não são deletadas, apenas desvinculadas enquanto a submeta está na lixeira. Questão em aberto (UC-GOAL-007): se dinheiro RESERVED/COMMITTED depois vira uma Transaction vinculada de verdade, remover a alocação manual equivalente é responsabilidade do usuário — sem isso, o mesmo valor conta duas vezes no progresso.
- **Transaction**: valor, tipo (receita/despesa, ambos no MVP), data, categoria, Account, **pagador**, **responsável econômico** (pode ser diferente do pagador), **SplitRule**, objetivo vinculado opcional (`goalId` — é o que forma o PAID de uma Goal), forma de pagamento (débito/crédito, só relevante quando a Account é um cartão). Visibilidade da transação é independente da visibilidade da Account usada e dos detalhes de split.
...
- **Account**: nome, tipo, contexto, visibilidade — independente da visibilidade de qualquer Transaction que a referencie. Cartão de crédito é um tipo de Account (limite, dia de fechamento/vencimento, conta de pagamento) com Fatura própria por ciclo; um mesmo cartão pode ter transações em débito (afeta a conta de pagamento na hora, nunca entra em fatura) ou crédito (entra no ciclo da fatura) — é uma escolha por transação, não do cartão (ver UC-FIN-011).
...
- **JRN-005 — Criar e acompanhar objetivo (com submetas)**: `UC-GOAL-001→UC-GOAL-007→UC-TASK-001/008→UC-TASK-003→UC-GOAL-007(GoalAllocation)→UC-GOAL-003`. Criar meta → relacionar submetas (Goal→Goal) e tarefas → completar tarefas → organizar o financeiro de submetas (RESERVED/COMMITTED como GoalAllocation manual; PAID vinculando uma Transaction real ao objetivo) → progresso atualiza automaticamente → completar meta (automático em 100% ou manual).
...
- Inventário: auth 6, calendar 7, finance 14, food 8, goals 8, groups 13 (8 UC-GROUP + 5 UC-PERM), tasks 8, today 2, users 4 — total 70 arquivos. Os 4 UCs novos de finance (UC-FIN-011 a 014: cartão/fatura, parcelamento, recorrência, painel financeiro) e o UC-GOAL-008 (ritmo/ideal por mês) vieram do aprofundamento de Finanças com referência no app Minhas Finanças — junto disso, receita e recorrência/parcelamento saíram do V2 e entraram no MVP (ver `docs/product/roadmap.md`).
...
- **Atenção**: UC-FIN-001 a 008 e UC-GOAL-001 a 006 são anteriores às PD-006/PD-007 e podem estar desatualizados em relação a elas (revisão deliberadamente adiada — ver PDs acima). UC-GOAL-007 já foi revisado e alinhado (o mecanismo de `PAID` mudou durante a prototipação — deixou de ser uma transição de estado da `GoalAllocation` e passou a ser sempre derivado de `Transaction.goalId`, ver PD-007 e domain-model.md).
...
- **Implementado até agora**, em `apps/web/src/prototype/`:
  - **Onboarding**: fluxo completo (JRN-001 + JRN-002 até o onboarding financeiro mínimo, UC-FIN-009) — cadastro, escolha/criação de grupo, perfil, as 3 telas de finança do grupo (dinheiro/caixinha, split, transparência), tela final.
  - **Home**: Home/Hoje básica com listas por domínio e filtro de contexto — critério de "o que precisa de atenção" ainda não definido (questão em aberto que bloqueia o design final, ver `information-architecture.md`).
  - **Tarefas**: lista com filtro de contexto, criação/edição, conclusão, prazo real (data ISO, não label solto), recorrência (flag simples, sem motor de geração de ocorrências).
  - **Calendário**: três visões — Agenda (lista agrupada por dia), Semana (grade hora-a-hora com blocos posicionados por horário/duração, linha de "agora"), Mês (grade com indicador de dia com compromisso). Recorrência real (diária/semanal/mensal) com exceções por ocorrência — editar ou excluir só uma ocorrência materializa uma exceção isolada, sem afetar as demais da série (`UC-CAL-007`). Tarefas com prazo aparecem na agenda/grade como item "dia inteiro" — integração experimental, sem UC formal ainda.
  - **Objetivos**: lista com progresso, submetas (`Goal→Goal`, um nível) como mini-cards com navegação em pilha dentro do próprio sheet (entrar numa submeta e voltar, sem empilhar sheet sobre sheet). Estados financeiros: `RESERVED`/`COMMITTED` são alocação manual; `PAID` é sempre derivado da soma das `Transaction`s vinculadas via `goalId` — nunca digitado à parte dentro do objetivo (ver correção registrada em `UC-GOAL-007`). Ideal por mês e aviso de ritmo (`UC-GOAL-008`).
  - **Finanças**: visão geral com economia mensal, despesas por categoria e ranking; Contas (inclui cartão de crédito com Fatura por ciclo); Transações com parcelamento e recorrência; forma de pagamento débito/crédito por transação num mesmo cartão (`UC-FIN-011`); vínculo opcional de uma transação a um Objetivo.
  - Componentes compartilhados relevantes além dos de onboarding: `Tile`, `DomainLabel`, `FlagChip`, `Avatar`, `VisibilityDot` (elemento de assinatura pra `PRIVATE`/`SHARED`/`GROUP`, presente em quase toda tela), `CategoryChip` + `palette.ts` (cor determinística por categoria/pessoa), `ContextFilterChips`, `PhoneFrame` (moldura de exibição do protótipo).
  - Estado é sempre local por tela (`useState` a partir de um `initial*` mockado) — não há store compartilhado entre telas. Navegar pra outra tela e voltar reseta o que foi editado nessa sessão; é o comportamento esperado do "estado local simples" pedido nas regras desta fase, não um bug.
- `docs/ui/` ainda **não existe** — nenhuma tela recebeu handoff formal ainda (nenhuma foi marcada `APPROVED FOR DEV`), mesmo com Home/Tarefas/Calendário/Objetivos/Finanças já implementados e testados no protótipo.
