...
- **Só extrair uma `interface`/`type` para um arquivo `types.ts`** (ou qualquer lugar importável) **quando ele já for, ou muito provavelmente for, referenciado por mais de um lugar.** Caso contrário — ex.: o tipo de um parâmetro usado numa única função —, definir inline. Não poluir `types.ts` com tipos de uso único. **Exceção: um tipo de entidade/domínio retornado por um `service.ts`/`repository.ts` de uma feature (ex.: `Group`) sempre vai em `features/<nome>/types.ts`**, mesmo antes de existir um segundo consumidor — esse tipo tende a atravessar a fronteira da feature (frontend consumindo via `index.ts`), diferente de um tipo de parâmetro de uso interno. Ver `apps/web/features/groups/types.ts` como referência.
...
- **Validação de formulários usa VeeValidate**, priorizando a Composition API da biblioteca (`useForm`, `defineField`/`useField`) em vez dos componentes `<Form>`/`<Field>` dela. **Zod** é o schema validator (via `@vee-validate/zod`, `toTypedSchema`) — mesma lib usada para validar payload de API (ver abaixo), sem duplicar duas bibliotecas de schema no projeto.
...
- **Lógica de negócio de uma feature (`features/<nome>/server/`) sempre em um arquivo `<nome>.service.ts`** (ex.: `group.service.ts`), nunca um arquivo por função (ex.: `createGroup.ts`) nem lógica solta em outro lugar dentro de `server/`. Um método de `service.ts` **não valida o payload em runtime** — o tipo do parâmetro já é o contrato. Ver `apps/web/features/groups/server/group.service.ts` como referência.
- **Validação de payload de uma rota da API usa `parseBody(event, schema)`** (`apps/web/server/utils/validation.ts`, auto-importado nas rotas do Nitro) — lê e valida o body com um schema **Zod** dentro do próprio arquivo de rota (`server/api/`), antes de chamar o service; lança um erro 400 se o payload for inválido. É ali, não no service, que a responsabilidade de validar em runtime vive. Ver `apps/web/server/api/groups.post.ts` como referência.
- **Acesso ao banco de dados (Drizzle) sempre através de um arquivo `<entidade>.repository.ts`** dentro de `features/<feature>/server/`, nunca diretamente de um `service.ts` — o service chama o repository, o repository chama `useDb()`/schema. **Um arquivo de repository é escopado por entidade "raiz" da feature, não por feature inteira nem por tabela individual**: uma entidade que depende estruturalmente de outra dentro da mesma feature — só existe ou é acessada no contexto dela, ex.: `Membership` e `Invitation` não fazem sentido fora de um `Group` — compartilha o repository dessa entidade raiz (`group.repository.ts` cobre `Group`, `Membership` e `Invitation`) em vez de ganhar um arquivo próprio. Já uma entidade de **outra feature** (ex.: `User`) nunca é consultada de dentro do repository de quem a consome, mesmo que hoje só exista um consumidor — a consulta vai no repository da feature dona (`features/users/server/users.repository.ts`), exposto pelo `index.ts` dela e importado de lá por quem precisar, mesma regra de fronteira que já vale pro resto do código de uma feature. Enforced via ESLint (`no-restricted-imports` bane `~/lib/db/*` fora de `*.repository.ts` dentro de `server/`; arquivos `*.test.ts` são isentos, já que testes legitimamente precisam de acesso direto ao banco para fixtures/assertions). Ver `apps/web/features/groups/server/group.repository.ts` (cobre `Group`, `Membership` e `Invitation`) e `apps/web/features/users/server/users.repository.ts` como referência.
- **Não definir um tipo de entidade/domínio só como alias de um tipo de vendor** (ex.: `typeof groups.$inferSelect` do Drizzle, ou reexportar `User` do `@supabase/supabase-js`) — isso acopla o resto do código à forma de dados de uma lib específica e dificulta migrar essa lib no futuro. Definir o tipo explicitamente (`interface`/`type` com os campos desejados), mesmo que a estrutura hoje coincida com a inferida pela lib.
- **Cada teste é responsável por criar os próprios dados de fixture** — evitar um `beforeEach` compartilhado entre vários testes de um `describe`; a criação (via helper local, se necessário) acontece dentro do próprio `it`. **Não fazer teardown/cleanup dos dados criados a menos que seja explicitamente pedido** — dados de teste acumulados no Postgres local não são um problema nesta fase do projeto. Para gerar dados de fixture nas tabelas do Drizzle, usar `@praha/drizzle-factory` (`defineFactory`) em vez de `insert` manual. **Nunca definir o factory dentro do próprio arquivo de teste** — ele vai em `features/<entidade>/server/<entidade>.factory.ts`, na feature dona da entidade que ele constrói (ex.: um factory da tabela `users` vai em `features/users/server/users.factory.ts`, mesmo que hoje só seja consumido pelos testes da feature `groups`), não na feature que o consome. Outra feature importa esse factory pelo `index.ts` da feature dona (`~/features/users`), nunca pelo caminho interno — mesma regra de "só index.ts é importável de fora" que já vale para o resto do código de uma feature. Ver `apps/web/server/api/groups.post.test.ts` e `apps/web/features/users/server/users.factory.ts` como referência.
- **Um teste que exercita uma função ou rota com efeito colateral no banco (ex.: um método de `service.ts`, ou uma rota que chama esse método) deve consultar o banco diretamente para confirmar o estado persistido, em vez de depender só do valor de retorno/response testado** — o retorno pode estar certo mesmo com um bug na escrita real (ou vice-versa). Vale tanto pro caminho de sucesso (a linha esperada existe, com os campos esperados) quanto pro caminho de rejeição (nenhuma linha foi criada). Ver `apps/web/server/api/groups/[groupId]/invitations.post.test.ts` como referência.
- **Toda rota autenticada (que chama `requireAuthUser`) precisa de um teste garantindo que ela retorna 401 quando a requisição não tem um token válido** — cada rota reafirma esse contrato por conta própria; não vale assumir que a cobertura de outra rota já garante isso. Ver `apps/web/server/api/groups.post.test.ts` como referência.
- **Teste de rota** (o handler do Nitro por trás de um arquivo em `server/api/`, cobrindo autenticação, parsing/validação do body e status code — não só a lógica de negócio, que já é coberta pelo teste do `service.ts`) **fica co-localizado junto da própria rota** (ex.: `server/api/groups.post.test.ts` para `server/api/groups.post.ts`), usando `setup()`/`fetch()` de `@nuxt/test-utils/e2e` para subir um servidor Nitro real e fazer a requisição HTTP de verdade, em vez de chamar o handler ou o `service.ts` diretamente. Exige `// @vitest-environment node` no topo do arquivo — o ambiente `nuxt` (usado pelo resto da suíte) é só para testes de composable/componente e quebra com `setup()`. Como o arquivo de teste roda no processo do vitest, fora da instância real do Nuxt que `setup()` sobe como subprocesso, `useRuntimeConfig()` não está disponível ali, e por consequência `useDb()`/`useSupabaseAdmin()`/`useSupabase()` (que dependem dele) também não. O setup compartilhado entre testes de rota (client Drizzle/Supabase montados lendo `process.env` diretamente, sem passar por `useRuntimeConfig()`; criação de sessão de usuário fixture; `setupRouteTest()` que encapsula o `setup()`) vive em `apps/web/tests/route-test-helpers.ts`, importado via `~/tests/route-test-helpers` — **nunca dentro de `server/api/`**, mesmo não sendo uma rota de verdade: qualquer `.ts` ali é varrido pelo build do Nitro, que quebra ao topar com um pacote só-de-build (`@nuxt/test-utils/e2e`) nesse grafo. Ver `apps/web/server/api/groups.post.test.ts` e `apps/web/server/api/groups/[groupId]/invitations.post.test.ts` como referência.
